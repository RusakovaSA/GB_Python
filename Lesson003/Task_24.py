...
garden = []
garden.append(random.sample(range(n), n))

print(f"Расположение кустов на грядке по кругу {garden}.")

shrub = int(input("Введите номер кустарника для собирающего модуля: "))
harvest = 0
i = shrub - 1
while i < n:
    if i == n-1:
        harvest = garden[i] + garden[i-1] + garden[0]
    if i == 0:
        harvest = garden[n-1] + garden[i+1] + garden[0]
    else:
        harvest = garden[i] + garden[i-1] + garden[i+1]
